...
{
    public sealed class OperationEndsWithConverter : BaseOperationConverter
    {
        public OperationEndsWithConverter(LinqContext context, IEnumerable<IRelationConverter> relationConverters) :
        ...
        {
            return clientType == ClientTypes.EntityFramework && operation == QueryOperations.EndsWith;
        }

        public override void Convert(Type entityType, string parameterName, string field, object value,
            QueryOperations operation, PropertyInfo property, QueryRelations relations)
        {
            var clause = $"{property.Name}.EndsWith(@{parameterName})";
            var clause1 = clause;
